    ...
    public class DataAccess
    {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                using (var reader = cmd.ExecuteReader())
                    while (reader.Read())
                        ...
                        reader.GetString(9), reader.GetString(3), reader.GetInt64(8), reader.GetInt32(5), reader.GetString(6)));
        ...
        public dynamic InsertAlbum (Album album, List<int> trackList)
        {
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                if (albums.Count == 0)
                {
                    cmd.CommandText = @"INSERT INTO ""MusicLibrary"".""Albums"" (""AlbumName"",""ArtistName"",""Year"",""Genre"",""AlbumArt"",""FolderCreationDate"") 
                                        VALUES (@AlbumName, @ArtistName, @Year, @Genre, @AlbumArt, @FolderCreationDate)";
                ...
                else
                {
                    cmd.CommandText = @"UPDATE ""MusicLibrary"".""Albums"" SET ""AlbumArt"" = @AlbumArt, ""FolderCreationDate"" = @FolderCreationDate, 
                                        ""Genre"" = @Genre WHERE ""AlbumName"" = @AlbumName AND ""ArtistName"" = @ArtistName AND ""Year"" = @Year";
                    cmd.ExecuteNonQuery();
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                if (double.TryParse(re, out a))
                {
                    if (a == 1)
                    {
                        ...
                        cmd.CommandText = @"INSERT INTO ""MusicFolder"".""SyncFailures"" (""Type"", ""ArtistName"", ""TrackName"", ""AlbumName"", ""NewTrackName"") VALUES (@type, @ArtistName, @TrackName, @AlbumName, @NewName)";
                        cmd.ExecuteNonQuery();
                    }
                    else if (a == 2)
                    {
